...
module DesignHuddleAdmin
  class ProjectsProjectIDPATCHRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DesignHuddleAdmin::ProjectsProjectIDPATCHRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DesignHuddleAdmin::ProjectsProjectIDPATCHRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@project_title.nil? && @project_title.to_s.length > 50
        invalid_properties.push('invalid value for "project_title", the character length must be smaller than or equal to 50.')
      end
    ...
    def valid?
      return false if !@project_title.nil? && @project_title.to_s.length > 50
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] project_title Value to be assigned
    def project_title=(project_title)
      if !project_title.nil? && project_title.to_s.length > 50
        fail ArgumentError, 'invalid value for "project_title", the character length must be smaller than or equal to 50.'
      end
    ...
    def hash
      [project_title, brand_id, brand_code].hash
    end
