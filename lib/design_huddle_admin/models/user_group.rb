  ...
  class UserGroup
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DesignHuddleAdmin::UserGroup` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DesignHuddleAdmin::UserGroup`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'user_group_id')
        self.user_group_id = attributes[:'user_group_id']
      end

      if attributes.key?(:'user_group_code')
        self.user_group_code = attributes[:'user_group_code']
      end

      if attributes.key?(:'user_group_name')
    ...
    def list_invalid_properties
      ...
      if @user_group_id.nil?
        invalid_properties.push('invalid value for "user_group_id", user_group_id cannot be nil.')
      end

      if @user_group_name.nil?
        invalid_properties.push('invalid value for "user_group_name", user_group_name cannot be nil.')
      end
    ...
    def valid?
      return false if @user_group_id.nil?
      return false if @user_group_name.nil?
    ...
    def hash
      [user_group_id, user_group_code, user_group_name, excluded].hash
    end
