...
module DesignHuddleAdmin
  class ProjectsProjectIDExportJobsJobIDGETSuccessOneOf1Data
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DesignHuddleAdmin::ProjectsProjectIDExportJobsJobIDGETSuccessOneOf1Data` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DesignHuddleAdmin::ProjectsProjectIDExportJobsJobIDGETSuccessOneOf1Data`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @date_completed.nil?
        invalid_properties.push('invalid value for "date_completed", date_completed cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @date_completed.nil?
    ...
    def hash
      [date_started, date_completed, completed, thumbnail_url, download_url].hash
    end
